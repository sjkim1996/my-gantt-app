...
export async function POST(req: Request) {
  try {
    ...
    const response = NextResponse.json({
      success: true,
      data: { id: user.id, role: user.role, label: user.label, team: (user as { team?: string }).team || '' },
    });
    writeSessionCookie(response, { id: user.id, role: user.role, label: user.label, team: (user as { team?: string }).team || '' });
    return response;
  } catch (error) {
